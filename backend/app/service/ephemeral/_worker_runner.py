# ========= Copyright 2025-2026 @ Eigent.ai All Rights Reserved. =========
# Licensed under the Apache License, Version 2.0 (the "License");
# you may not use this file except in compliance with the License.
# You may obtain a copy of the License at
#
#     http://www.apache.org/licenses/LICENSE-2.0
#
# Unless required by applicable law or agreed to in writing, software
# distributed under the License is distributed on an "AS IS" BASIS,
# WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
# See the License for the specific language governing permissions and
# limitations under the License.
# ========= Copyright 2025-2026 @ Eigent.ai All Rights Reserved. =========

"""
Code that runs INSIDE an ephemeral worker environment.

It executes a single HTTP request against the in-process FastAPI app using TestClient,
and writes a simple streaming protocol to stdout:

  EIGENT_META <json>\n
  EIGENT_CHUNK <base64>\n   (0..N times)
  EIGENT_DONE\n
"""

import base64
import json
import os
import sys
from pathlib import Path
from typing import Any


def _b64decode(s: str) -> bytes:
    return base64.b64decode(s.encode("utf-8"))


def _b64encode(b: bytes) -> str:
    return base64.b64encode(b).decode("utf-8")


def _load_request() -> dict[str, Any]:
    """
    Load the serialized request for this worker.

    Priority:
      1. EIGENT_EPHEMERAL_REQUEST_FILE: JSON file path
      2. EIGENT_EPHEMERAL_REQUEST_B64: base64-encoded JSON (fallback for E2B-style usage)
      3. stdin (base64-encoded JSON) - legacy fallback
    """
    request_file = os.environ.get("EIGENT_EPHEMERAL_REQUEST_FILE")
    if request_file:
        data = Path(request_file).read_text(encoding="utf-8")
        return json.loads(data)

    payload_b64 = os.environ.get("EIGENT_EPHEMERAL_REQUEST_B64", "").strip()
    if not payload_b64 and not sys.stdin.closed and not sys.stdin.isatty():
        payload_b64 = sys.stdin.read().strip()
    if not payload_b64:
        raise SystemExit("Missing EIGENT_EPHEMERAL_REQUEST_FILE or EIGENT_EPHEMERAL_REQUEST_B64")

    return json.loads(_b64decode(payload_b64).decode("utf-8"))


def main() -> None:
    req: dict[str, Any] = _load_request()

    # IMPORTANT: ensure workers do not recursively gateway.
    os.environ["EIGENT_EPHEMERAL_GATEWAY_ENABLED"] = "false"

    from app import api  # local import so env is set first
    from app.component.environment import env
    from app.router import register_routers
    from starlette.testclient import TestClient

    # Ensure routers are registered in worker (main.py does this in the gateway process).
    prefix = env("url_prefix", "")
    register_routers(api, prefix)

    method = str(req["method"]).upper()
    path = str(req["path"])
    query_string = str(req.get("query_string") or "")
    url = path + (f"?{query_string}" if query_string else "")
    headers = dict(req.get("headers") or {})
    body = _b64decode(str(req.get("body_b64") or ""))

    with TestClient(api) as client:
        with client.stream(method, url, headers=headers, content=body) as r:
            meta = {
                "status_code": r.status_code,
                "headers": dict(r.headers),
                "media_type": r.headers.get("content-type"),
            }
            sys.stdout.write("EIGENT_META " + json.dumps(meta) + "\n")
            sys.stdout.flush()

            for chunk in r.iter_bytes():
                if not chunk:
                    continue
                sys.stdout.write("EIGENT_CHUNK " + _b64encode(chunk) + "\n")
                sys.stdout.flush()

            sys.stdout.write("EIGENT_DONE\n")
            sys.stdout.flush()


if __name__ == "__main__":
    main()

